...
w("-- pas.");
w("");
w("insert into public.salons (slug, nom, ville, code_postal, rue, type, latitude, longitude, demo) values");
REELS.forEach((s, i) => {
  const fin = i === REELS.length - 1 ? "" : ",";
  w("  (" + t(s.slug) + ", " + t(s.name) + ", " + t(s.city) + ", " + t(s.postalCode) + ", " + t(s.street) + ", " + t(s.type) + ", " + n(s.lat) + ", " + n(s.lng) + ", false)" + fin);
});
...
w("");
w("insert into public.salons (slug, nom, ville, code_postal, rue, type, latitude, longitude, demo, description, telephone) values");
DEMOS.forEach((s, i) => {
  const fin = i === DEMOS.length - 1 ? "" : ",";
  w("  (" + t(s.slug) + ", " + t(s.name) + ", " + t(s.city) + ", " + t(s.postalCode) + ", " + t(s.street) + ", " + t(s.type) + ", " + n(s.lat) + ", " + n(s.lng) + ", true,");
  w("   " + t(s.description) + ",");
  w("   " + t(s.phone) + ")" + fin);
});
w("on conflict (slug) do update set");
